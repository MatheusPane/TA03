...
@section('title', 'Referensi Makanan Pokok')
    ...
    <div class="content-card" style="border-radius: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.1);">
        <div class="card-header-custom" style="background: linear-gradient(135deg, var(--primary), var(--primary-light)); color: white; padding: 15px;">
            <h4 style="font-weight: 700; margin: 0;">
                Referensi Makanan Pokok
            </h4>
            <a href="{{ route('ref_makanan_pokok.create') }}" class="btn btn-light" style="border-radius: 10px;">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped align-middle" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('ref_makanan_pokok.edit', $item->id) }}" 
                                       class="btn btn-sm btn-warning" title="Edit">
                                        Edit
                                    </a>
                                    <form action="{{ route('ref_makanan_pokok.destroy', $item->id) }}" 
                                          method="POST" class="d-inline" 
                                          onsubmit="return confirm('Yakin hapus makanan pokok ini?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="text-center text-muted">Belum ada data makanan pokok.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
