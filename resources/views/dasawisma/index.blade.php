...
<div class="main-content p-4">
    <div class="content-card" style="border-radius: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);">
        <div class="card-header-custom" style="border-bottom: 2px solid rgba(0, 0, 0, 0.05); padding: 15px; display: flex; justify-content: space-between; align-items: center;">
            <h4 style="font-weight: 700; color: var(--text-dark); margin: 0;">Data Dasawisma</h4>
            @if(Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Pengurus'))
                <a href="{{ route('dasawisma.create') }}" class="btn btn-primary" style="border-radius: 10px;">
                    <i class="bi bi-plus-circle"></i> Tambah Dasawisma
                </a>
            @endif
        </div>
        ...
        <div class="p-3">
            <div class="table-responsive">
                <table id="dasawismaTable" class="table table-bordered table-striped align-middle" style="width:100%">
...
$(document).ready(function() {
    ...
    window.Laravel = {
        canEdit: @json(Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Pengurus')),
        canDelete: @json(Auth::user()->hasRole('Admin')),
        routes: {
            edit: '{{ route('dasawisma.edit', ':id') }}',
            destroy: '{{ route('dasawisma.destroy', ':id') }}',
            anggota_create: '{{ route('dasawisma_anggota.create', ':id') }}',
            anggota_index: '{{ route('dasawisma_anggota.index', ':id') }}'
        }
    };

    const table = $('#dasawismaTable').DataTable({
        language: {
        ...
        buttons: [
            ...
            { extend: 'excel', text: 'Excel', className: 'btn btn-success btn-sm', title: 'Data Dasawisma' },
            { extend: 'pdf', text: 'PDF', className: 'btn btn-danger btn-sm', title: 'Data Dasawisma' },
            { extend: 'print', text: 'Print', className: 'btn btn-info btn-sm', title: 'Data Dasawisma' }
        ],
        columnDefs: [
            { targets: 0, width: '50px', className: 'text-center' },
            { targets: 1, width: '200px' },
            { targets: 7, orderable: false, width: '200px', className: 'text-center' }
        ],
        order: [[1, 'asc']],
        drawCallback: function() {
            $('#dasawismaTable tbody tr').each(function() {
                ...
                if (window.Laravel.canDelete) {
                    buttons += `
                        <form action="${window.Laravel.routes.destroy.replace(':id', id)}" method="POST" class="d-inline" onsubmit="return confirm('Yakin hapus dasawisma ini?')">
                            @csrf @method('DELETE')
